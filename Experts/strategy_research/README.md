...
Orkestrasi resmi Fase 2. Split kronologis 75% in-sample / 25%
out-of-sample, parameter DIBEKUKAN (`lookback=20, stop_z=3.0` + filter
tren `long_window=200`), cek out-of-sample SATU KALI.

Sekarang menerima simbol lain lewat argumen — parameter tetap beku
persis sama untuk semua simbol, ini memperluas pertanyaan yang sama ke
instrumen lain, bukan kesempatan tuning ulang:
```
python run_research.py                  # default: EURUSD
python run_research.py AUDUSD USDCAD     # simbol lain, parameter sama
```

**Aturan tidak boleh dilanggar:** begitu angka out-of-sample keluar
untuk suatu simbol, JANGAN ubah parameter untuk simbol itu lalu
jalankan ulang.

## Status Fase 2

Hasil sejauh ini:
- **EURUSD** (`run_research.py`): in-sample bagus (PF 1,29, Sharpe
  1,18), **out-of-sample negatif** (PF 0,98, Sharpe -0,07, 98 trade —
  di bawah ambang signifikan).
- **Validasi silang** (`cross_instrument_check.py`, full-data, belum
  displit): AUDUSD (379 trade, PF 1,17, Sharpe 0,70) dan USDCAD (341
  trade, PF 1,21, Sharpe 0,68) konsisten dengan EURUSD full-data (PF
  1,21, Sharpe 0,87) — pola generalisasi lintas instrumen. GBPUSD (13
  trade) dan USDJPY (18 trade) sampelnya terlalu kecil untuk berarti
  apa pun, meski USDJPY terlihat "bagus" (Sharpe 5,16 — kemungkinan
  besar cuma noise sampel kecil).
- **Belum diketahui**: apakah AUDUSD/USDCAD JUGA bertahan lewat
  pemisahan in-sample/out-of-sample seperti EURUSD, atau ikut merosot.
  Ini pertanyaan kunci berikutnya, dijawab lewat
  `python run_research.py AUDUSD USDCAD`.

### `cross_instrument_check.py`

Jalankan strategi yang PERSIS SAMA (parameter beku, tidak disetel
ulang per simbol) di GBPUSD, USDJPY, AUDUSD, USDCAD — bukti independen
soal apakah edge EURUSD generalisasi ke instrumen lain atau EURUSD
outlier sendirian. Menangani `pip_size` khusus untuk USDJPY (0,01,
bukan 0,0001) supaya biaya transaksi tidak salah skala.
...
Cara uji:
```
python cross_instrument_check.py
```
